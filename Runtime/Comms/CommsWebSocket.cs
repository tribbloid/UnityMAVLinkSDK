...
namespace MAVLinkSDK.Comms
{
    public class WebSocket : CommsBase, ICommsSerial, IDisposable
    {
        ...
        public ClientWebSocket client = new();
        ...
        private MemoryStream readMemoryStream = new();
        private int readCursor = 0;
        private SemaphoreSlim clientSemaphoreSlim = new(1);
        ...
        private async void RunReader()
        {
            while (IsOpen)
                try
                {
                    ...
                    {
                        var stringdata = Encoding.ASCII.GetString(buffer.Array, 0, data.Count);
                        log.Info(stringdata);

                        if (stringdata[0] == '0')
                        {
                            var match = new Regex(@"""sid"":""([^""]+)""");
                        ...
                        {
                            _ = client.SendAsync(
                                new ArraySegment<byte>("40/MAVControl,".ToCharArray().Select(a => (byte)a).ToArray()),
                                WebSocketMessageType.Text, true, CancellationToken.None);

                            _ = client.SendAsync(
                                new ArraySegment<byte>("5".ToCharArray().Select(a => (byte)a).ToArray()),
                                WebSocketMessageType.Text, true, CancellationToken.None);
                        }
                    }
                    else
